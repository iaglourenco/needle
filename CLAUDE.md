...
`hookmode` → local HTTP (`server.rs`, axum, POST `/event`) → `state::transition(current, event)` (pure state machine, see `state.rs`) → `db::upsert_session` (SQLite, `db.rs`) → `tray::on_session_changed` (`tray.rs`): re-reads the session's Claude Code transcript for cost/model, updates tray tooltip, emits `session-updated` to the webview, fires an OS notification if the new state warrants it and `Settings.notifications_enabled` is on. `session-removed` is emitted only from `main.rs` — on the 24h stale-purge in `spawn_cleanup_job` and on manual deletion via the `delete_session` command — never from `tray.rs`.
